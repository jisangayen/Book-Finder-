...
export default function BookCard({ book }) {
  return (
    <Link to={`/book/${book.id.replace("/works/", "")}`}>
      <div className="relative w-40 flex-shrink-0 cursor-pointer group transition-transform duration-300 hover:scale-105">
        ...
        <div className="rounded-xl overflow-hidden shadow-lg border-2 border-transparent group-hover:border-purple-500 transition-all duration-300 bg-gray-800">
          <img
            src={book.cover}
            alt={book.title}
            className="w-40 h-56 object-cover rounded-t-xl"
          />
          <div className="p-3">
            <h3 className="text-base font-bold text-white mb-1 line-clamp-1">
              {book.title}
            </h3>
            <p className="text-xs text-red-600">{book.author}</p>
          </div>
        </div>
